...
best_arch_para = [list(list_empty) for i in range(generation_max)] #保存每代中每个个体训练后的参数
...
def convert_to_circuit(image):
    ...
    for i, value in enumerate(values):
        if value:
            ...
            circuit.append(cirq.ry(value * np.pi)(qubits[i]))#编码为Rx
    return circuit
...
def create_quantum_model(chromosome_i):
    ...
    for j in range(0,genomeLength,2):
        if chromosome_i[j]==0:
...
def train_qnn(chromosome_i,i,EPOCHS,plt1=False):
    ...
    #将线路加载至keras模型中
    #创建keras模型
    init = tf.keras.initializers.RandomNormal(mean=5, stddev=3, seed=44)
    ...
    model.compile(
        ...
        optimizer=tf.keras.optimizers.Adam(lr=0.005,epsilon=1e-8),
    ...
    best_arch_para[i]=PQC1.symbol_values()
    if(plt1==True):
...
#根据染色体构建量子神经网络
        qnn_results = train_qnn(chromosome[i],i,EPOCHS=7)
...
def rotation():
    ...
    for i in range(0,popSize):
        ...
        for j in range(0,genomeLength):
            ...
            if fitness[i]<fitness[int(best_chrom[generation-1])]:

# if chromosome[i,j]==0 and chromosome[best_chrom[generation],j]==0:

                if chromosome[i,j]==0 and chromosome[int(best_chrom[generation-1]),j]==1:

# 旋转角0.03pi

                    if nqpv[i,j,0]*nqpv[i,j,1]>0:
                        delta_theta=-theta
                    if nqpv[i,j,0]*nqpv[i,j,1]<0: 
                        delta_theta=theta
                    if nqpv[i,j,1]==0:
                ...
                if chromosome[i,j]==1 and chromosome[int(best_chrom[generation-1]),j]==0:
                ...
                if chromosome[i,j]==1 and chromosome[int(best_chrom[generation-1]),j]==1:
...
#全干扰交叉
def all_cross():
    qpv_best = qpv[int(best_chrom[generation-1])]
    qpv_ori = qpv
    k = 0 #递增数
    for j in range(0,genomeLength):
        for i in range(0,popSize):
            qpv[i,j]=qpv_ori[i-(k%popSize),j]
        k+=1
        
        
    qpv[int(best_chrom[generation-1])] = qpv_best #将最优染色体还原，精英保留
...
def Q_GA():
    ...
    while (generation<generation_max-1):#此处有逻辑问题，两次覆盖generation=0的情况！
...
print(max_index)
final_qnn_results = train_qnn(best_arch[max_index],max_index,EPOCHS=15,plt1=True)

with open('best_mnist.txt','w') as f:
    f.write("best_acc: \n")
...
ax_x = range(0,generation_max-1)
plt.plot(ax_x,best_acc,label="best_acc")
...
plt.plot(ax_x,av_fitness,label="avg_acc")
plt.legend()
plt.savefig("./double_fig.png")
plt.show()
