...
images_test = []
for f in glob.iglob("/home/lyy/327/zpx/copy1/test_burke1/*"):
    images_test.append(np.asarray(Image.open(f).convert('L')))
for f in glob.iglob("/home/lyy/327/zpx/copy1/test_nimitz1/*"):
    images_test.append(np.asarray(Image.open(f).convert('L')))
...
model.compile(
    ...
    optimizer=tf.keras.optimizers.Adam(lr=0.01,epsilon=1e-08),
...
plt.plot(qnn_history.history['val_hinge_accurac61796909y'])
...
plt.grid(True)
plt.savefig('./final-1.png')
plt.show()

qnn_results = model.evaluate(x_test_tfcirc, y_test)
print(model.weights)
print(tfq.util.get_circuit_symbols(model_ciircuit))
print("------------------------")
print(PQC1.symbol_values())
