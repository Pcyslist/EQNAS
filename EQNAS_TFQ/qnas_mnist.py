...
best_acc = np.empty([generation_max-1])
best_arch = np.empty([generation_max-1, genomeLength])
...
def train_qnn(chromosome_i,i,EPOCHS,plt1=False):
    ...
    #将线路加载至keras模型中
    #创建keras模型
    #init = tf.keras.initializers.RandomNormal(mean=5, stddev=3, seed=32)
...
print(max_index)
final_qnn_results = train_qnn(best_arch[max_index],max_index,EPOCHS=10,plt1=True)
