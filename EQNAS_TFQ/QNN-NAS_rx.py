...
#读取文件夹中所有图像并转灰度
images = []
for f in glob.iglob("./data/Burke/*"):
    images.append(np.asarray(Image.open(f).convert('L')))
for f in glob.iglob("./data/Nimitz/*"):
    images.append(np.asarray(Image.open(f).convert('L')))
#将图像进行归一化
...
images_test = []
for f in glob.iglob("./data/test_burke/*"):
    images_test.append(np.asarray(Image.open(f).convert('L')))
for f in glob.iglob("./data/test_nimitz/*"):
    images_test.append(np.asarray(Image.open(f).convert('L')))
...
def train_qnn(chromosome_i,i,EPOCHS):
    ...
    model.compile(
        ...
        optimizer=tf.keras.optimizers.SGD(lr=0.05,momentum=0.9,decay=0.0004,nesterov=False),
